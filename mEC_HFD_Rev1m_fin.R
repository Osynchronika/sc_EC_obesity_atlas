...
# Load data
setwd("~/Desktop/HI-MAG_OLGA/SeqData/Cellranger/2021_04_15_M_HFD_Rev_1m/analysis")

data.dir = "~/Desktop/HI-MAG_OLGA/SeqData/Cellranger/2021_04_15_M_HFD_Rev_1m/data"
...
print("Mark diet")
for (i in c(1,4,7,10,12,14,16)) {
  tissue[[i]]$diet = "chow"
}
for (i in c(2,5,8,13,15,17)) {
  tissue[[i]]$diet = "Western"
}
for (i in c(3,6,9,11,18)) {
  tissue[[i]]$diet = "Rev"
}

print("Mark time")
for (i in c(1,2,4,5,7,8,10,12,13,14,15,16,17)) {
  tissue[[i]]$time = "4m"
}
for (i in c(3,6,9,11,18)) {
  tissue[[i]]$time = "1m"
...
FeaturePlot(combined, features = c("Flt1", "Pecam1","Cdh5",# EC
                                   ...
                                   "Prox1", "Pdpn","Lyve1", #LECs
                                   "Hba-a1", "Hba-a2","Hbb-bs",
                                   "Ptprc", "Mrc1","Cd52"), # immune
...
combined <- RenameIdents(object = combined,`11` = "Lymph-EC", `15` = "FB",`21` = "FB-heart",
                               `17` = "Pericyte-brain", `21` = "Pericyte-lung",`11` = "SMC") 
...
combined = readRDS("combined/combined_merged_organs_singlet.rds")


# select cluster by hand
Idents(combined) = "seurat_clusters"
plot <- DimPlot(combined, reduction = "umap", pt.size = 0.5)
select.cells <- CellSelector(plot = plot) # 
Idents(combined, cells = select.cells) <- "mural"
...
plot <- DimPlot(combined, reduction = "umap", pt.size = 0.5)
select.cells <- CellSelector(plot = plot) # 
Idents(combined, cells = select.cells) <- "LECs"
...
non.ecs = WhichCells(combined, idents = c("FB","mural"))
...
saveRDS(combined, "combined_merged_organs_ECs.rds")

Idents(combined) = combined$celltype
DimPlot(combined, reduction = "umap", group.by = "organ",label = TRUE, raster = F)  # 10.5 x 10
...
Idents(combined) = combined$celltype
saveRDS(subset(combined,idents = "LECs"), 'lymphatic_subset.rds')
